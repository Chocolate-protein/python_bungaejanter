...
class login:
    def __init__(self, window):
        ...
        self.back_img = tk.PhotoImage(file="imgs/back.png").subsample(3, 3)
...
#######################################
    ...
    def main_bot_talk(self,e):
        ...
        for i in range(len(talk_fetchall)):
            ...
            self.main_talk.user_label = ttk.Label(self.main_talk_frame, text=f"{self.main_talk.user_list[0]}", border=0,
                                                  background="white")
            ...
            # 상품 이미지
            main_talk_sale_img = Image.open(io.BytesIO(self.main_talk.user_list[11]))
            ...
            self.main_talk_sale_name = tk.Label(self.main_talk_frame, text=self.main_talk.user_list[1])
################################################
    # 찜 화면과 최근 본 상품 화면
...
###################################################
    ...
    def talk_pay_btn(self, e):
        ...
        self.price_x = 335
        self.price_y = 32
        if len(self.talk_sale_list[5]) == 6:
            self.pay_product_price.place(x=self.price_x - 10, y=self.price_y)
        else:
        ...
        self.total_x = 346
        self.total_y = 176
        if len(str(self.total)) == 4:
            self.total_price.place(x=self.total_x, y=self.total_y)
        elif len(str(self.total)) == 5:
            self.total_price.place(x=self.total_x - 10, y=self.total_y)
        elif len(str(self.total)) == 6:
            self.total_price.place(x=self.total_x - 20, y=self.total_y)
    ...
    def main_recent_click(self, e):
        ...
        for i in range(len(self.recent_sale)):
            ...
            self.mini_price = ttk.Label(self.recent_in_scroll_mini,
                                        text=self.recent_sale[i].winfo_children()[1].cget("text"))
            self.mini_price.configure(font=("맑은 고딕", 14, "bold"), background="white")
            self.mini_price.place(x=0, y=240)
            # 상품명
            self.recent_name = truncate_text_recent(self.recent_sale[i].winfo_children()[2].cget("text"))
            ...
            self.recent_in_scroll_mini.bind("<Button-1>", lambda event, sinfo_list=s_list: self.sale_click(event, sinfo_list))
    ...
    def main_like_click(self, e):
        for widget in self.like_in_scroll.winfo_children():
            ...
            # widget.forget()       # destroy 하면 제품을 다시는 열 수 없음
        openFrame(self.main_recent_win)
...
######################################
    ...
    def recent_search(self, e):
        ...
        for widget in self.sale_frame.winfo_children():
            ...
            widget.forget()  # 관련 상품창의 위젯들이 안사라짐
...
##################################################
    ...
    def pay_click(self, e, frame):
        ...
        self.price_x = 335
        self.price_y = 32
        if len(sales_dic[self.click_label][5]) == 6:
            self.pay_product_price.place(x=self.price_x - 10, y=self.price_y)
        else:
            self.pay_product_price.place(x=self.price_x, y=self.price_y)
    ...
    def like_click(self, e):
        print(e.widget.master.winfo_children()[3].cget("text")[:-3])
        if self.like_bool:
