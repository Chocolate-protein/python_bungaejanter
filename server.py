...
con = pymysql.connect(host=HOST, user='root', password="0000", port=3306, db="team5",charset='utf8')
# con = pymysql.connect(host='localhost', user='root', password="0000", port=9900, db="team5",charset='utf8')
cur = con.cursor()
...
class UserManager:
    ...
    def main_recent_click(self, s_name, s_price):
        ...
        # try:
        #     if msg == "main_recent_click_name":
        #         print("이름 검색")
        #         sql = f"SELECT s_name FROM team5.sales WHERE s_name LIKE '%{click_widget}%'"
        #     else:
        #         print("이미지 검색")
        #         sql = f"SELECT s_img FROM team5.sales WHERE s_name = '{click_widget}';"
        #
        #     cur.execute(sql)
        #     mr_list = cur.fetchall()
        #
        #     serialized_data = pickle.dumps(mr_list)
        #
        #     # 데이터 길이 전송
        #     data_length = len(serialized_data)
        # except:
